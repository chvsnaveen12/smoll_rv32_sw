...
int main() {
    ...
    *SPI_CLK_DIV = 200;
    ...
    while (retries-- > 0) {
        sd_cmd(55, 0, 0xFF);
        if (sd_cmd(41, 0x40000000, 0xFF) == 0x00)
            break;
    }
    ...
    *SPI_CLK_DIV = 1;
    ...
    uart_puts("Jump Addr: ");
    uart_put_hex32(jump_addr);
    uart_puts("\n\r");
    uart_puts("Transfers: ");
    uart_put_hex32(num_transfers);
    uart_puts("\n\r");
    ...
    for (uint32_t i = 0; i < num_transfers; i++) {
        ...
        uart_puts("Loading LBA: ");
        uart_put_hex32(start_lba);
        uart_puts(" -> ");
        uart_put_hex32(dest_addr);
        uart_puts(" (");
        uart_put_hex32(num_blocks);
        uart_puts(" blocks)\n\r");

        if (sd_read_blocks(start_lba, num_blocks, (uint8_t *)(uintptr_t)dest_addr) != 0) {
        ...
        uint8_t *p = (uint8_t *)(uintptr_t)dest_addr;
        for (int k = 0; k < 16; k++) {
            uart_put_hex8(p[k]);
            uart_putc(' ');
        }
    ...
    while (UART_RX_AVAIL)
        (void)UART_RX_DATA; // flush stale RX
