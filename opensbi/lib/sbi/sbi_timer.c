...
#ifdef SMOLL_RV32
	/* We don't care about doing 64-bit multiplication as the result is only 64-bits */ 
	double_result_t delta_result = multiply_unsigned_with_loops((ulong)timer_dev->timer_freq, (ulong)units);
	delta = ((u64)(delta_result.high) << 32) | (u64)(delta_result.low);
#else
